...
dotenv_1.default.config({
    path: process.env.DOTENV_CONFIG_PATH ?? path_1.default.join(__dirname, "..", ".env"),
});
...
async function main() {
    ...
    console.log("╚══════════════════════════════════════════╝");
    console.log("");
    const opencode = new opencode_1.OpenCodeClient(OPENCODE_HOST, OPENCODE_PORT, OPENCODE_SERVER_PASSWORD);
    // 1. Intentar conectar con un opencode serve ya existente
    const isAlreadyRunning = await opencode.healthCheck();
    if (isAlreadyRunning) {
        console.log(`✓ OpenCode server detectado en ${OPENCODE_HOST}:${OPENCODE_PORT}`);
    }
    else {
        // 2. Si no está corriendo, lo iniciamos nosotros
        console.log(`→ OpenCode server no detectado. Iniciando opencode serve...`);
    ...
    const notifier = new notifier_1.EmailNotifier();
    const app = (0, server_1.createServer)(opencode, resolvedModel, notifier, API_SECRET);
    ...
    if (MONITOR_INTERVAL_MINUTES > 0) {
        ...
        setInterval(async () => {
            try {
                ...
                const { text } = await opencode.chat([{ type: "text", text: "Responde solo con la palabra OK" }], resolvedModel);
                if (!text.trim()) {
                    const msg = "Respuesta vacía del modelo (posible sesión expirada)";
                    console.error(`[${new Date().toISOString()}] [ALERTA] ${msg}`);
...
// ──────────────────────────────────────────────
function startOpencodeServe(port, hostname, password) {
...
// ──────────────────────────────────────────────
async function waitForOpencode(client, maxSeconds) {
    ...
    while ((Date.now() - start) / 1000 < maxSeconds) {
        ...
        if (ok) {
            console.log(`✓ OpenCode server listo (intento #${attempt})`);
            return true;
        }
        if (attempt % 5 === 0) {

...
class EmailNotifier {
    constructor() {
        ...
        if (this.smtpHost && this.smtpUser && this.smtpPass) {
            ...
            console.log("⚠ Notificaciones por correo NO configuradas (faltan SMTP_HOST, SMTP_USER o SMTP_PASS en .env)");
        }
    }
    /**
     ...
     */
    async sendCredentialExpiredAlert(errorDetails) {
        if (!this.configured || !this.transporter) {
            console.error("[ALERTA] Credenciales de ChatGPT expiradas, pero el correo no está configurado.");
        ...
        if (now - this.lastAlertTime < this.COOLDOWN_MS) {
            console.log("[ALERTA] Credenciales expiradas (correo ya enviado recientemente, cooldown activo)");
            return;
        }
        try {

...
export class OpenCodeClient {
  ...
  async createSession(title?: string): Promise<OpenCodeSession> {
    const res = await this.http.post<OpenCodeSession>("/session", {
  ...
  async getDefaultModel(): Promise<string> {
    try {
      ...
      const data = res.data as { default?: Record<string, string> };
      const defaults = data.default ?? {};
      // Buscamos el default de OpenAI primero, luego anthropic
      if (defaults["openai"]) return `openai/${defaults["openai"]}`;
      if (defaults["anthropic"]) return `anthropic/${defaults["anthropic"]}`;
